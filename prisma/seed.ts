...
async function main() {
  ...
  const headlight = await prisma.product.create({
    data: {
      ...
      description:
        "Premium LED headlight assembly with auto-leveling and dynamic beam pattern.",
      price: 15000,
      originalPrice: 18000,
      type: "SPARE_PART",
  ...
  const audio = await prisma.product.create({
    data: {
      ...
      description:
        "Modern touchscreen audio system with Bluetooth and Apple CarPlay/Android Auto.",
      price: 45000,
      originalPrice: 52000,
      type: "ACCESSORY",
      availabilityStatus: "AVAILABLE",
